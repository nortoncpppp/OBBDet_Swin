...
def eval_mAP(root_dir, use_07_metric=False):
    ...
    output_files_path = os.path.join(root_dir, "Recognition")
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        for txt_file in dr_files_list:
            ...
            for line in lines:
                try:
                    tmp_class_name, confidence, x1, y1, x2, y2= line.split()
                    # if not recognition:
                    #     tmp_class_name = 'plane'
                    # else:
                    #     pass
                except ValueError:
                    error_msg = "Error: File " + txt_file + " in the wrong format.\n"
    ...
    with open(output_files_path + "/output.txt", 'w') as output_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            Fa = tp[:]
            for idx, val in enumerate(tp):
                Fa[idx] = float(fp[idx]) / (fp[idx] + tp[idx] + 1e-6)
            # print(prec)
    ...
    with open(output_files_path + "/output.txt", 'a') as output_file:
        ...
        gt_counter_all_class = 0
        for class_name in sorted(gt_counter_per_class):
            output_file.write(class_name + ": " + str(gt_counter_per_class[class_name]) + "\n")
            gt_counter_all_class += gt_counter_per_class[class_name]
    ...
    with open(output_files_path + "/output.txt", 'a') as output_file:
        ...
        count_true_positives_all = 0
        det_counter_all_class = 0
        result_plane_r = 0
        W = 1/40
        for class_name in sorted(dr_classes):
            ...
            if class_name in gt_classes:
                CR = count_true_positives[class_name]/gt_counter_per_class[class_name]
                WR = 1-count_true_positives[class_name]/det_counter_per_class[class_name]
                if CR == 0:
                    result_plane_r_i=0
                else:
                    result_plane_r_i = W*((1+math.pow(1.5,2))*CR*(1-WR)/(CR+math.pow(1.5,2)*(1-WR)))
                result_plane_r += result_plane_r_i
            else:
                pass
            count_true_positives_all += count_true_positives[class_name]
            det_counter_all_class += n_det
        text1 = "result_plane_r:" + str(result_plane_r) + "\n"
        output_file.write(text1)

        output_file.write("\n")
        # DR = count_true_positives_all/ gt_counter_all_class
        # FAR = (det_counter_all_class-count_true_positives_all) / det_counter_all_class
        # Result_plane_d = (1+math.pow(1.5,2))*DR*(1-FAR)/(DR+math.pow(1.5,2)*(1-FAR))
        # text = "DR:" + str(DR) + "\n"
        # text += "FAR:" + str(FAR) + "\n"
        # text += "Result_plane_d:" + str(Result_plane_d) + "\n"
        #
        # Result_plane = 0.3*Result_plane_d+0.7*result_plane_r
        # text +="Result_plane:"+str(Result_plane)
        # output_file.write(text)
    """
    ...
    return mAP,result_plane_r

def eval_mAP1(root_dir, use_07_metric=False):
    MINOVERLAP = 0.5  # default value (defined in the PASCAL VOC2012 challenge)
    no_animation = False
    no_plot = False
    quiet = False
    ...
    if not os.path.exists(TEMP_FILES_PATH):  # if it doesn't exist already
        os.makedirs(TEMP_FILES_PATH)
    output_files_path = os.path.join(root_dir, "detection")
    if os.path.exists(output_files_path):  # if it exist already
        # reset the output directory
        shutil.rmtree(output_files_path)
    ...
    with open(output_files_path + "/output.txt", 'a') as output_file:
        ...
        for class_name in sorted(dr_classes):
            ...
            text += " (tp:" + str(count_true_positives[class_name]) + ""
            text += ", fp:" + str(n_det - count_true_positives[class_name]) + ")\n"
            DR = count_true_positives[class_name] / gt_counter_all_class
            FAR = (n_det - count_true_positives[class_name])/n_det
            Result_plane_d = (1+math.pow(1.5,2))*DR*(1-FAR)/(DR+math.pow(1.5,2)*(1-FAR))
            text += "DR:"+str(DR)+"\n"
            text += "FAR:"+str(FAR)+"\n"
            text += "Result_plane_d:" + str(Result_plane_d) + "\n"
    ...
    return mAP,Result_plane_d
if __name__ == "__main__":
    mAP,RE_R = eval_mAP('/media/think/D8EA260719113F00/0data/GaoJing/ps/lmy/geo/mAP')
    mAP1,RE_D= eval_mAP1('/media/think/D8EA260719113F00/0data/GaoJing/ps/lmy/geo/mAP')
    output_files_path = '/media/think/D8EA260719113F00/0data/GaoJing/ps/lmy/geo/mAP'
    with open(output_files_path + "/res.txt", 'a') as output_file:
        Results = 0.3*RE_D+0.7*RE_R
        text = "RE_R = "+str(RE_R)+"\n"
        text += "RE_D = " + str(RE_D) + "\n"
        text += "Results = "+str(Results)
        output_file.write(text)